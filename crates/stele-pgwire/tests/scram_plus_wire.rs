...
//! end-to-end over real sockets (STL-297, broadened to SHA-384/512 by STL-330).
//!
...
//! RFC 5929 §4.1 selects that hash from the certificate's signature algorithm, so
//! the suite mints leaves across the SHA-2 family — ECDSA-SHA-256 (the STL-297
//! floor), ECDSA-SHA-384, and RSA-SHA-512 — and proves each binds and
//! authenticates with the matching digest (STL-330).
//!
...
/// Which signature algorithm the test CA signs the leaf with — and therefore the
/// leaf's `signatureAlgorithm`, which RFC 5929 §4.1 reads to pick the
/// channel-binding digest.
#[derive(Clone, Copy)]
enum CaSig {
    /// ECDSA P-256 → `ecdsa-with-SHA256` leaf → SHA-256 binding (the STL-297 floor).
    EcdsaSha256,
    /// ECDSA P-384 → `ecdsa-with-SHA384` leaf → SHA-384 binding (STL-330).
    EcdsaSha384,
    /// RSA-2048 → `sha512WithRSAEncryption` leaf → SHA-512 binding (STL-330).
    RsaSha512,
}
...
///
/// It exists because rcgen's `ring` backend (the workspace's TLS provider — no
/// `aws_lc_rs`) can *generate* ECDSA / Ed25519 keys but not RSA ones; it can only
/// *load* a supplied RSA key. Embedding one fixed key keeps the test deterministic
/// and adds no dependency — the alternative, an `rsa` keygen dev-dependency on the
/// most dependency-sensitive surface, is exactly what STL-330 set out to avoid.
/// It authenticates nothing real: a localhost test CA, regenerated by any
/// `openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048` if ever needed.
...
/// Mint a fresh ECDSA-SHA-256 PKI — the common case (see [`mint_pki_with`]).
fn mint_pki(test: &str) -> Pki {
    mint_pki_with(test, CaSig::EcdsaSha256)
}

/// Mint a fresh PKI for one test under a scratch directory, with the CA signing
/// the leaf using `ca_sig`. The leaf carries both a `localhost` DNS SAN (for the
/// raw-socket client) and a `127.0.0.1` IP SAN (for the `tokio-postgres` client
/// connecting by address), and a P-256 key for the TLS handshake; its
/// *signature* algorithm — what RFC 5929 binds against — is the CA's, so varying
/// `ca_sig` is what exercises SHA-256 / SHA-384 / SHA-512 channel binding.
fn mint_pki_with(test: &str, ca_sig: CaSig) -> Pki {
...
// our `tokio-rustls` stream. It exposes the `tls-server-end-point` value via the
// shared [`endpoint_binding`] cert-hash logic, so it tracks the leaf's signature
// algorithm (SHA-256/384/512) just as the server and libpq do.
// ---------------------------------------------------------------------------
    ...
    fn channel_binding(&self) -> ChannelBinding {
        self.0
            ...
            .map_or_else(ChannelBinding::none, |cert| {
                ChannelBinding::tls_server_end_point(endpoint_binding(cert.as_ref()))
            })
    }
}
...
/// The `tls-server-end-point` binding (RFC 5929 §4.1) as a client derives it: the
/// leaf certificate hashed with the digest its signature algorithm names, across
...
/// The `tls-server-end-point` binding for the certificate the handshake presented.
...
/// Drive a stock `tokio-postgres` client with `channel_binding=require` against a
/// server whose leaf is signed per `ca_sig`, and round-trip a query. `require`
/// makes the connect fail unless the server advertised PLUS *and* the binding the
/// driver independently computed matched — a true interop check of the wider hash.
async fn assert_tokio_postgres_plus_interop(test: &str, ca_sig: CaSig) {
    let pki = mint_pki_with(test, ca_sig);
    let addr = spawn_server(&pki, "alice", test_password()).await;
    let connector = RustlsConnect(Arc::new(client_config(&pki.ca_pem)));
    let password = test_password();
    let conn_str = format!(
    ...
    let (client, connection) = tokio_postgres::connect(&conn_str, connector)
        .await
        .expect("SCRAM-SHA-256-PLUS should authenticate with channel binding");
    tokio::spawn(connection);
    let rows = client.query("SELECT 1", &[]).await.expect("query");
    let v: i32 = rows[0].get(0);
    assert_eq!(v, 1);
...
#[tokio::test]
async fn tokio_postgres_negotiates_plus_with_a_sha384_cert() {
    // An ECDSA-P384 / SHA-384-signed leaf: PLUS must bind with SHA-384 end-to-end,
    // and a stock driver's independent SCRAM negotiates it (STL-330).
    assert_tokio_postgres_plus_interop("interop-sha384", CaSig::EcdsaSha384).await;
}

#[tokio::test]
async fn tokio_postgres_negotiates_plus_with_a_sha512_cert() {
    // An RSA / SHA-512-signed leaf: PLUS must bind with SHA-512 end-to-end (STL-330).
    assert_tokio_postgres_plus_interop("interop-sha512", CaSig::RsaSha512).await;
}
...
#[tokio::test]
async fn correct_channel_binding_authenticates_with_a_sha384_cert() {
    // The leaf is ECDSA-P384 / SHA-384, so its binding is the SHA-384 of the cert
    // DER — 48 bytes, not the 32-byte SHA-256 the STL-297 floor produced (STL-330).
    let pki = mint_pki_with("plus-ok-sha384", CaSig::EcdsaSha384);
    let addr = spawn_server(&pki, "alice", test_password()).await;
    let mut stream = tls_connect(addr, client_config(&pki.ca_pem)).await;
    let cbind = peer_endpoint_binding(&stream);
    assert_eq!(cbind.len(), 48, "a SHA-384-signed leaf binds with SHA-384");
    write_startup(&mut stream, "alice").await;
    let result = scram_exchange(
        &mut stream,
        "SCRAM-SHA-256-PLUS",
    ...
    assert_eq!(
        result,
        Ok(()),
        "genuine SHA-384 channel binding authenticates"
    );
}

#[tokio::test]
async fn correct_channel_binding_authenticates_with_a_sha512_cert() {
    // The leaf is RSA / SHA-512, so its binding is the SHA-512 of the cert DER —
    // 64 bytes. Proves the RSA-signed leg of the wider-hash support (STL-330).
    let pki = mint_pki_with("plus-ok-sha512", CaSig::RsaSha512);
    let addr = spawn_server(&pki, "alice", test_password()).await;
    let mut stream = tls_connect(addr, client_config(&pki.ca_pem)).await;
    let cbind = peer_endpoint_binding(&stream);
    assert_eq!(cbind.len(), 64, "a SHA-512-signed leaf binds with SHA-512");
    write_startup(&mut stream, "alice").await;
    let result = scram_exchange(
        &mut stream,
        "SCRAM-SHA-256-PLUS",
    ...
    assert_eq!(
        result,
        Ok(()),
        "genuine SHA-512 channel binding authenticates"
    );
}
