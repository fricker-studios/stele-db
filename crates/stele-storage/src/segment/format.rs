...
pub enum ColumnId {
    ...
    /// table's segments ([`FORMAT_VERSION`] v3); absent otherwise.
    ValidFrom = 7,
...
impl ColumnId {
    ...
    /// [`Self::ValidTo`]; use [`Self::schema`] to get the full ordered set for
    /// a given valid-time policy rather than iterating `ALL` directly when the
    /// opt-in columns matter.
