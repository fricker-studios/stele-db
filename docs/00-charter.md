...
| **Security & auditability (unified pillar)** | The audit-native primitives *are* security primitives: immutability is tamper-resistance, provenance is who-did-what-when, time-travel is forensics, and a **hash-chained, Merkle-proof commit log** is third-party-verifiable tamper-evidence — an auditor verifies inclusion/consistency *without trusting the operator* ([ADR-0026](adr/0026-verifiable-audit-log.md)). On top of that, table-stakes controls are first-class, not afterthoughts: encryption in transit + at rest (KMS/BYOK), strong authN, RBAC + row/column security, access auditing, and Rust's memory-safety/exploit resistance. See [10 — Security & Compliance](10-security-and-compliance.md) and [ADR-0018](adr/0018-security-auditability-pillar.md). |
