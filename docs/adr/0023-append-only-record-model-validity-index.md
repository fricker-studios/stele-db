...
- **Retractions are persisted records, never inferences.** Version adjacency can only reconstruct *supersession* closes; a **delete is a close with no successor**, and adjacency inference would silently resurrect the row across the deletion gap (insert → updates → delete@t3 → re-insert@t4 would infer the deleted version open until t4 — see [16 §12](../16-bitemporal-semantics.md#12-deletes-retractions--the-deletion-gap)). Retraction records are therefore durable at three layers: the **WAL** (always); **persisted into segments at flush as payload-less tombstone rows** (key, target `sys_from`, `closed_at`, `seq`, closing provenance), making the segment store self-contained for a from-scratch rebuild even after WAL truncation; and **validity-index checkpoints**, so routine recovery is *checkpoint + WAL tail*.
