...
- **v0.3 (STL-316)** refines the summary from segment to *row-group* granularity: the footer additionally carries one such coalesced-union summary per row-group (format v14, gated by `FOOTER_FLAG_RG_VALID_INTERVALS`, emitted alongside the per-segment one), so the scan skips an individual row-group whose coverage gaps at `v` even when the segment as a whole cannot be pruned — a production flush bounds row-groups (STL-197), so a single scatter-heavy row-group can carry windows spanning the timeline. This mirrors how STL-173 refined the system-axis zone maps from segment to row-group granularity; the per-segment summary stays as the cheap first cut. Same advisory/derived posture (no separate structure to rebuild).
