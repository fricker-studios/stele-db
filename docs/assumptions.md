...
| A32 | **Storage lifecycle / tiering** | **System-time-driven tiered archival** (hot→Standard→IA/Glacier Instant→Deep Archive), keeping every byte (distinct from retention/deletion). Resident zone maps prune before rehydrating; **explicit async `RESTORE`** for frozen data; **engine-native, pluggable** (S3 Intelligent-Tiering optional). Controls append-only cost growth. | `CONFIRMED` | [ADR-0021](adr/0021-storage-lifecycle-tiered-archival.md) |
| A33 | **Clock sync (distributed)** | **NTP required** on all nodes (operator-enforced preflight + monitoring) — but a baseline, not the guarantee. **Hybrid Logical Clocks** for causally-consistent cross-node system-time; **max-skew self-fencing** (fail-safe); PTP/cloud time-sync optional to tighten the bound. Single-node unaffected. | `CONFIRMED` | [ADR-0022](adr/0022-clock-synchronization-and-ordering.md) |
| A34 | **Record model (append-only)** | **No stored `sys_to`.** Records carry only `sys_from`; `sys_to` is materialized **once** into a derived, rebuildable **validity index** at supersession. Atomic close+open; per-key serialization point. | `CONFIRMED` | [ADR-0023](adr/0023-append-only-record-model-validity-index.md) |
| A35 | **Time representation** | **µs, int64, UTC, `+∞` sentinel**, half-open intervals, per-commit monotonic **sequence** for same-tick total order (avoids the int64-ns 2262 cliff). | `CONFIRMED` | [ADR-0024](adr/0024-time-representation.md) |
| A36 | **Valid-time indexing** | System-time prunes via **zone maps**; valid-time via a **dedicated index** (scatter defeats zone maps), optional valid-time clustering. | `CONFIRMED` | [ADR-0025](adr/0025-valid-time-indexing.md) |
| A37 | **Verifiable audit log (headline)** | Tamper-evidence elevated to a headline pillar: **hash-chained log ~v0.2**, **Merkle inclusion/consistency proofs ~v0.5** (was v0.7); third-party-verifiable; crypto-shred preserves proofs. | `CONFIRMED` | [ADR-0026](adr/0026-verifiable-audit-log.md) |
| A38 | **Temporal axes** | **Bitemporal now**, record + catalog shape **reserved for an optional 3rd `decision/knowledge` axis** (off by default). | `CONFIRMED` | [02 §2](02-architecture.md#2-the-bitemporal-record-model) |
| A39 | **Temporal access control** | As-of reads enforce the **current** access policy by default (time-travel is not a privilege-escalation channel); optional as-of-policy mode. | `CONFIRMED` | [10 §6](10-security-and-compliance.md#6-authorization) |
| A40 | **Coalescing** | **No auto-coalesce on write** — facts stored as asserted (audit fidelity); coalesced views on demand; split/merge must not change results. | `CONFIRMED` | [16 §9](16-bitemporal-semantics.md) |
| A41 | **Oracle / CP** | Primary correctness oracle = **DuckDB** (+ Postgres). Distributed Stele is **CP** (reject writes under partition; never reconcile divergent histories). | `CONFIRMED` | [06 §10](06-testing-strategy.md), [ADR-0006](adr/0006-distribution-later-shared-storage.md) |
