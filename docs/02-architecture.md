...
1. **Per-row transaction provenance** (Must, v0.2): who/what/when wrote each version. Cheap, always-on. It is a **queryable surface** (v0.3, [STL-247]): three hidden pseudo-columns — `_stele_txn_id`, `_stele_committed_at`, `_stele_principal` — read a row's provenance inline in a `SELECT` (and a `WHERE`), the Postgres system-column posture (excluded from `SELECT *` and `\d`). They resolve over live, `AS OF`, and range reads — each returning the version's *own* writing provenance, drawn from the version metadata, never a user column. See [sql-grammar.md](sql-grammar.md#provenance-pseudo-columns-stl-247). The `_stele_principal` value is the **connection's identity** (STL-300): the SCRAM-verified user under `auth = "scram"`, the unauthenticated startup `user` under `"trust"`, stamped per statement under the engine lock (a direct, embedded writer defaults to the server identity `stele`) — the [security](10-security-and-compliance.md) line's authenticated identity threaded into write provenance without changing the surface.
