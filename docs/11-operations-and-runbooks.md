...
| `stele_scan_segments_scanned_total`, `stele_scan_segments_pruned_zone_total`, `stele_scan_segments_pruned_bloom_total`, `stele_scan_segments_pruned_superseded_total`, `stele_scan_segments_pruned_valid_total`, `stele_scan_row_groups_scanned_total`, `stele_scan_row_groups_pruned_zone_total` | counter | Snapshot-scan pruning accounting (STL-146/STL-173/STL-238/STL-241): how much sealed data reads actually touch vs. skip — including per-segment bloom-filter skips on point business-key probes and per-segment valid-time interval-summary skips on `FOR VALID_TIME AS OF` reads (the backdated scatter case zone maps cannot prune). |
