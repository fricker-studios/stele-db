...
- **Linking:** each binary statically links the Rust standard library and all crate dependencies, but dynamically links the platform's system C runtime — glibc on the Linux `-gnu` targets, `libSystem` on macOS, the UCRT on Windows — so none of the shipped archives is a fully static (musl) build. The Linux archives therefore carry a **glibc floor**: the minimum glibc is whatever the `ubuntu-latest` release runner ships (glibc 2.39 / Ubuntu 24.04 LTS at the time of writing), so a host with older glibc fails with a `version 'GLIBC_x.y' not found` loader error. For a fully static, runtime-dependency-free deployment use the container image (§2) — the canonical no-toolchain path; a portable musl Linux build is a tracked future enhancement (STL-350), of most value to the `brew install` path (STL-259).
