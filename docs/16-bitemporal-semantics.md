...
The engine combines the *ranged* axis (the un-ranged axis follows the single-table range convention — a system range is valid-agnostic, a valid range pins the system snapshot); the both-axes sequenced combination remains a follow-up, as does the **unbounded** sequenced form with *no* qualifier (a plain join over valid-time tables still reads all system-live rows, [STL-218], rather than the interval combination). A **materialized (CTE / derived) input** ([STL-242]) has no temporal axis to range, so it is read once at the statement snapshot and treated as a degenerate `[−∞, +∞)`-live relation ([STL-349]) — the identity for the interval intersection, so it contributes no narrowing and a joined tuple's period comes from the ranged *base* sides (at least one input must be a base table; a range over a join of only materialized inputs has no axis to range and is rejected). A materialized side is admitted only under the `INNER` shape — the `LEFT` / `SEMI` / `ANTI` interval *difference* over an unbounded `[−∞, +∞)` period has no meaningful endpoints, a tracked follow-up. The differential oracle is `crates/stele-engine/tests/bitemporal_join_range_oracle.rs` (an independent breakpoint-sweep reference for the difference, with teeth that catch a union-not-intersection and an all-or-nothing-not-fragmenting mistake, plus a CTE-side workload).
