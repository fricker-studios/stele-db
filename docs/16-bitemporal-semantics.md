...
**The interval (sequenced) form (engine: STL-344).** A `FOR { SYSTEM_TIME | VALID_TIME } { FROM a TO b | BETWEEN a AND b }` range over a join produces the **intersected intervals on the ranged axis** over the queried window — the interval generalization of the consistent-snapshot rule above. Each input is range-scanned and a joined tuple's period is the **intersection** of its matched versions' intervals on that axis, `[max(from), min(to))`: a pair whose intervals never overlap does not join (the pointwise rule, swept over every instant in the queried window — `[a, b)` for the half-open `FROM..TO`, `[a, b]` for the closed `BETWEEN`), and the intersected endpoints are exposed on the output. The engine intersects the *ranged* axis (the un-ranged axis follows the single-table range convention — a system range is valid-agnostic, a valid range pins the system snapshot); the both-axes sequenced intersection and the `LEFT`/`SEMI`/`ANTI` interval-difference forms remain follow-ups, as does the **unbounded** sequenced form with *no* qualifier (a plain join over valid-time tables still reads all system-live rows, [STL-218], rather than the interval intersection). A **materialized (CTE / derived) input** ([STL-242]) has no temporal axis to range, so it is read once at the statement snapshot and treated as a degenerate `[−∞, +∞)`-live relation ([STL-349]) — the identity for the interval intersection, so it contributes no narrowing and a joined tuple's period comes from the ranged *base* sides; at least one input must therefore be a base table (a range over a join of only materialized inputs has no axis to range and is rejected), and the materialized side reflects the snapshot it was materialized at, not the queried interval. The differential oracle is `crates/stele-engine/tests/bitemporal_join_range_oracle.rs`.
