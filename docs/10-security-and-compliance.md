...
- **Channel binding over TLS (STL-297, STL-330):** on a TLS connection whose
  certificate yields a `tls-server-end-point` binding (RFC 5929), the server
  advertises `SCRAM-SHA-256-PLUS` first and folds the certificate hash into the
  `c=` check, so a man-in-the-middle that terminates TLS with a different
  certificate cannot relay a captured proof. RFC 5929 §4.1 takes the hash from the
  certificate's signature algorithm, so the binding tracks the SHA-2 family —
  SHA-256, SHA-384, or SHA-512 — and a client (libpq) computing it the same way
  derives the identical bytes (STL-330 lifted the original SHA-256-only floor).
  The RFC downgrade rule is enforced: over a PLUS-advertising channel a `y` gs2
  flag (the client believing the server has no channel binding) is refused. Off
  TLS — or for a certificate whose signature algorithm names no SHA-2 digest
  (legacy MD5/SHA-1, or non-hash schemes like Ed25519) — only plain SCRAM is
  offered, and a `p=…` demand is refused as before. The plain floor still
  authenticates over TLS for a client that opts out with `n`.
- **Deliberate v0.3 floor, filed as a follow-up:** client-side SCRAM in
  `stele shell` (STL-296) — the other v0.3 channel-binding follow-up,
  `tls-server-end-point` for SHA-384/512-signed certs, shipped in STL-330. The
  authenticated identity reaches
